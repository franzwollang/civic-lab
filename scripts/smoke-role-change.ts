...
async function main() {
  ...
  try {
    ...
    // Restore steward via HTTP (session-bound actor)
    ...
    const unauth = await app.request("/api/users/user-bob/roles", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ roles: ["contributor"] }),
    });
    if (unauth.status !== 401) {
      throw new Error(`anon role change expected 401, got ${unauth.status}`);
    }
