...
async function main() {
  ...
  // Word-boundary checks: `includes("DescriptiveArtifact")` also matches
  // `RedirectDescriptiveArtifact`.
  if (
  ...
  if (!/\bRedirectDescriptiveArtifact\b/.test(routes)) {
    throw new Error("missing RedirectDescriptiveArtifact");
  }
  if (!/\bRedirectLegacyRedTeam\b/.test(routes)) {
