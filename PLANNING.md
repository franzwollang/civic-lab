...
| Auth | Seed users + **header impersonation** (M8); real auth → M9 residual |
| Toolchain | **pnpm 9.15.9** (`packageManager`); `pnpm build` + `pnpm test:smoke` green (29/29) |

**Next build:** **M9 Policy** (`in progress` — Charter + reputation scaffolding landed). Remaining: anti-gaming board-hide/audit polish, real-auth hooks. Editor MVP residuals and evidence bridge remain available as side work.
...
| **M9 — Policy** | `in progress` | Charter living `owner_merge_only` artifact ✓; reputation advisory board ✓; remaining: anti-gaming board-hide/audit; real auth |
...
- Next: **M9** Policy residuals (anti-gaming board-hide / real auth); editor MVP residuals + evidence bridge remain open.
