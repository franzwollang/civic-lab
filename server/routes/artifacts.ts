...
async function handleGetArtifact(c: {
...
async function handleListRevisions(c: {
  req: { param: (k: string) => string };
}) {
  const id = c.req.param("artifactId") ?? c.req.param("pageId");
...
async function handleCreateRevision(c: {
...
async function handlePatchArtifact(c: {
  req: { param: (k: string) => string; json: () => Promise<unknown> };
}) {
  const id = c.req.param("artifactId") ?? c.req.param("pageId");
...
async function handleCreateArtifact(c: {
...
/**
 * Artifact / page / section / evidence-registry HTTP surface
 * (CONCEPT primary `/api/artifacts`; legacy `/api/pages` kept).
 */
