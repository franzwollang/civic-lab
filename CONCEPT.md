...
**Model→forecast implications:** model claims may carry `links` entries `{ kind: "implies_forecast", claim_id }` pointing at empirical forecast claims. Create/update rejects those links on non-model types and when the target is missing or not a forecast. Artifact claims panels render a read-only model→forecast implication DAG from loaded claims. **Advisory score propagation:** resolved implied forecasts contribute Brier / log / skill (vs baseline) onto the model as an artifact-scoped advisory snapshot (n, means, per-edge contributions); public-board eligibility still requires n≥20 (§5.9). Cross-collection reputation rollups of implication scores remain deferred.
...
| Model→forecast graph | Links + artifact-scoped DAG UI + advisory score propagation; reputation rollup deferred |
